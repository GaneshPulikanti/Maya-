...
export default function SharedChatView({ sessionId, onBackToApp }) {
  ...
  const speakMessageText = async (text, msgId) => {
    ...
    // Try premium backend TTS first if token is available
    ...
    if (token) {
      try {
        ...
        if (res.status === 200 && res.data?.size > 0) {
          ...
          audio.onerror = (e) => {
            console.error("SharedChatView TTS playback error, falling back to SpeechSynthesis:", e)
            cleanup()
        ...
        console.warn("SharedChatView backend TTS failed, falling back to SpeechSynthesis:", err)
      }
    }
