...
class LLMService:
    ...
    async def generate_response_stream(
        ...
        If a Groq API error occurs, it can trigger a backup LLM attempt.
        ...
        :param use_backup: Toggles the smaller, lightweight backup model for quick recovery
        """
        model_to_use = settings.BACKUP_LLM if use_backup else settings.PRIMARY_LLM
        logger.info(f"Initiating completion stream with model: {model_to_use}")
        
        # Construct the complete message payload with the system prompt prepended
        full_payload = [{"role": "system", "content": system_prompt}] + messages
        
        try:
            # Query Groq asynchronously with streaming enabled
            ...
            logger.error(f"Groq API error encountered on model {model_to_use}: {str(ge)}")
            if not use_backup:
                logger.warning("Attempting automatic failover to the backup LLM model...")
                # Recursively call with the backup model flag enabled
                async for chunk in self.generate_response_stream(messages, system_prompt, use_backup=True):
                    yield chunk
            else:
    ...
    async def generate_conversation_title(self, first_message: str) -> str:
        ...
        Sequentially tries fast robust models like llama-3.1-8b-instant and llama-3.3-70b-versatile.
        """
        prompt = (
